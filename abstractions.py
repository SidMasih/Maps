...
def user_reviewed_restaurants(user, restaurants):
    ...
    return [restaurant for restaurant in restaurants if restaurant_name(restaurant) in names]
...
def make_restaurant(name, location, categories, price, reviews):
    ...
    for review_list in reviews:
        rating.append(review_rating(review_list))
    # END Question 1
    return {
        'name': name,
        'location': location,
...
def restaurant_mean_rating(restaurant):
    ...
    # BEGIN Question 2
    sum_rating = 0.0
    for rating in restaurant_ratings(restaurant):
        sum_rating = sum_rating + rating

    return sum_rating / restaurant_num_ratings(restaurant)
    # END Question 2
